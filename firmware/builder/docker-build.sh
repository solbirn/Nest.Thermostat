...
# Mount custom certificate if provided
CERT_MOUNT=""
if [ -n "${NEST_CUSTOM_CERT_PATH:-}" ] && [ -f "${NEST_CUSTOM_CERT_PATH}" ]; then
    echo "[→] Mounting custom CA certificate: $NEST_CUSTOM_CERT_PATH"
    CERT_MOUNT="-v ${NEST_CUSTOM_CERT_PATH}:/custom-cert/ca.pem:ro"
fi
