...
DEFAULT_API_URL="https://frontdoor.nest.com"
...
print_header() {
  ...
  echo -e "${BOLD}${CYAN}╔═══════════════════════════════════════════════════════════════╗${NC}"
  echo -e "${BOLD}${CYAN}║                                                               ║${NC}"
  echo -e "${BOLD}${CYAN}║             ${BOLD}Nest Firmware Builder${CYAN}                   ║${NC}"
  echo -e "${BOLD}${CYAN}║                                                               ║${NC}"
  echo -e "${BOLD}${CYAN}║                      ${YELLOW}Nest${CYAN}                           ║${NC}"
  echo -e "${BOLD}${CYAN}║                                                               ║${NC}"
  echo -e "${BOLD}${CYAN}╚═══════════════════════════════════════════════════════════════╝${NC}"
  echo
}
...
Options:
  ...
  --public-ssl             Use Mozilla's CA bundle for public SSL certificates
...
setup_dependencies() {
  ...
  # Convert logo.png to PPM format if needed
  if [ -f "deps/logo.png" ] && [ -x "deps/convert_logo.sh" ]; then
    print_info "Converting logo.png to PPM format..."
    bash deps/convert_logo.sh || {
      print_warning "Logo conversion failed - will use existing logo_nest_clut224.ppm"
    }
  fi
...
configure_build() {
  ...
  elif [ "$PUBLIC_SSL" = true ]; then
    echo "  SSL CA:              Mozilla CA bundle (public SSL)"
  else
    echo "  SSL CA:              Default (Nest CA)"
  fi
...
build_firmware_multi_gen() {
  ...
  if [ "$NEED_CUSTOM_BUILD" = true ]; then
    ...
    cd "$SCRIPT_DIR"
    print_info "Configuring Nest settings..."

    ENTRY_URL="$API_URL"
    if [[ ! "$ENTRY_URL" =~ /nest/entry$ ]]; then
      ENTRY_URL="${ENTRY_URL}/nest/entry"
    fi
    ...
    if [ -f "$ROOTME_SCRIPT" ]; then
      ...
      # For hosted mode or default API URL, use the default Nest certificate
      if [ "$HOSTED_MODE" = true ] || [ "$API_URL" = "https://frontdoor.nest.com" ]; then
...
-----END CERTIFICATE-----"
        ...
        print_success "Loaded default Nest CA certificate for embedding"
      # Custom cert provided via --custom-cert flag
      elif [ -n "$CUSTOM_CERT_PATH" ]; then
        if [ ! -f "$CUSTOM_CERT_PATH" ]; then
          print_error "Custom certificate not found: $CUSTOM_CERT_PATH"
          exit 1
        fi
        if ! grep -q 'BEGIN CERTIFICATE' "$CUSTOM_CERT_PATH"; then
          print_error "File does not appear to be a PEM certificate: $CUSTOM_CERT_PATH"
          exit 1
        fi
        CA_CERT_CONTENT=$(cat "$CUSTOM_CERT_PATH")
        USE_CUSTOM_CERT=true
        print_success "Loaded custom CA certificate from: $CUSTOM_CERT_PATH"
      # For self-hosted mode with custom API URL, try to load custom certificate
      elif [ -f "/server/certs/ca-cert.pem" ] && [ "$PUBLIC_SSL" != true ]; then
...
sed -i '/\/bin\/nest/d' /tmp/1/etc/init.d/rcS
...
ROOTME_EOF
        ...
        # Download Mozilla's CA bundle for public SSL support
        print_info "Downloading Mozilla CA certificate bundle..."
        MOZILLA_CA_URL="https://curl.se/ca/cacert.pem"
        CA_BUNDLE_PATH="$SCRIPT_DIR/deps/mozilla-ca-bundle.pem"
        
        if [ ! -f "$CA_BUNDLE_PATH" ] || [ "$FORCE_BUILD" = true ]; then
          curl -fsSL "$MOZILLA_CA_URL" -o "$CA_BUNDLE_PATH"
          print_success "Downloaded Mozilla CA bundle ($(du -h "$CA_BUNDLE_PATH" | cut -f1))"
        else
          print_info "Using cached Mozilla CA bundle"
        fi
        
        # Embed the full CA bundle in the rootme script
        cat >> "$ROOTME_SCRIPT" << 'ROOTME_EOF'
cat > /tmp/1/etc/ssl/certs/ca-bundle.pem << 'CA_CERT_EOF'
ROOTME_EOF

        cat "$CA_BUNDLE_PATH" >> "$ROOTME_SCRIPT"

        cat >> "$ROOTME_SCRIPT" << 'ROOTME_EOF'
CA_CERT_EOF

# Create symlinks for compatibility with various libraries
mkdir -p /tmp/1/usr/lib/ssl /tmp/1/usr/share/ssl /tmp/1/etc/pki/tls/certs /tmp/1/nestlabs/share/certs
ln -sf /etc/ssl/certs/ca-bundle.pem /tmp/1/usr/lib/ssl/cert.pem
ln -sf /etc/ssl/certs/ca-bundle.pem /tmp/1/etc/ssl/cert.pem
ln -sf /etc/ssl/certs/ca-bundle.pem /tmp/1/etc/pki/tls/certs/ca-bundle.crt
ln -sf /etc/ssl/certs/ca-bundle.pem /tmp/1/etc/ssl/certs/ca-certificates.crt

# CRITICAL: nlclient uses its own hardcoded certificate path
# Copy the CA bundle to where nlclient expects it
cp /tmp/1/etc/ssl/certs/ca-bundle.pem /tmp/1/nestlabs/share/certs/ca-bundle.pem

# Set global environment variables for SSL
if ! grep -q "CURL_CA_BUNDLE" /tmp/1/etc/profile; then
  echo "export CURL_CA_BUNDLE=/etc/ssl/certs/ca-certificates.crt" >> /tmp/1/etc/profile
  echo "export SSL_CERT_FILE=/etc/ssl/certs/ca-certificates.crt" >> /tmp/1/etc/profile
fi

# Ensure these are set for services started by init
if ! grep -q "CURL_CA_BUNDLE" /tmp/1/etc/init.d/rcS; then
  sed -i '2i export CURL_CA_BUNDLE=/etc/ssl/certs/ca-certificates.crt' /tmp/1/etc/init.d/rcS
  sed -i '3i export SSL_CERT_FILE=/etc/ssl/certs/ca-certificates.crt' /tmp/1/etc/init.d/rcS
fi

ROOTME_EOF
        print_success "Embedded Mozilla CA bundle for public SSL support"
      fi
